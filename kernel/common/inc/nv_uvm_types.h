...
typedef struct
{
    ...
    // TODO: Bug 1764955: Remove field when per-peer CE selection is implemented
    // in the UVM driver.
    NvU32 cePceMask;
...
typedef struct UvmGpuP2PCapsParams_tag
{
    ...
    NvU32 optimalNvlinkReadCEs;  // A mask with the optimal CEs to be used for P2P
                                 // reads over NVLink
    NvU32 optimalNvlinkWriteCEs; // A mask with the optimal CEs to be used for P2P
                                 // writes over NVLink

    NvU32 directAccess    :1;
