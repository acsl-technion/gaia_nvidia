...
typedef struct nv_linux_state_s {
    ...
    /* lock for mmap contexts, used only by unix rm */
    struct semaphore mmap_lock;
 ...
 */
static inline NvU64 nv_compress_nvlink_addr(struct pci_dev *pci_dev, NvU64 addr)
{
    NvU64 addr47 = addr;

#if defined(NVCPU_PPC64LE) && defined(NV_PNV_PCI_GET_NPU_DEV_PRESENT)
    if (pnv_pci_get_npu_dev(pci_dev, 0))
    {
        addr47 = addr & ((1ULL << 43) - 1);
        WARN_ON(addr47 & (1ULL << 42));
...
static inline NvU64 nv_expand_nvlink_addr(struct pci_dev *pci_dev, NvU64 addr47)
{
    NvU64 addr = addr47;
