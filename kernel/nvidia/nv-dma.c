...
)
{
    ...
    else
    {
        *priv = dma_map;
        ...
        {
            for (i = 0; i <  (dma_map->contiguous ? 1 : dma_map->page_count); i++)
                va_array[i] = nv_compress_nvlink_addr(dma_map->dev, va_array[i]);
        }
#if defined(NV_PNV_PCI_GET_NPU_DEV_PRESENT)
        /*
         * On systems that support NVLink sysmem links, check the assumption that
         * PCIe and NVLink DMA mappings are always equivalent. See Bug 1920398.
         */
        else if (pnv_pci_get_npu_dev(dma_map->dev, 0))
        {
            for (i = 0; i <  (dma_map->contiguous ? 1 : dma_map->page_count); i++)
                WARN_ON(va_array[i] != nv_compress_nvlink_addr(dma_map->dev, va_array[i]));
        }
#endif
    }
...
)
{
    ...
    *priv = dma_map->pages;
    ...
    {
        if (nv->nvlink_sysmem_links_enabled)
            va_array[0] = nv_expand_nvlink_addr(dma_map->dev, va_array[0]);
        WARN_ON(dma_map->mapping.contig.dma_addr != va_array[0]);
        nv_dma_unmap_contig(dma_map);
    }
    else
    {
...
)
{
    ...
    NvU64 i;
    nv_alloc_t *at = *priv;
    NvBool contig = NV_ALLOC_MAPPING_CONTIG(at->flags);
...
NV_STATUS NV_API_CALL nv_dma_unmap_peer
(
    nv_state_t *nv,
    NvU64       page_count,
    NvU64      *va
